  ...
  new GoogleStrategy(
    {
      ...
      clientSecret: process.env.GOOGLE_CLIENT_SECRET,
      callbackURL: process.env.GOOGLE_REDIRECT_URI,
      passReqToCallback: true,  // Pass request object
      ...
      try {
        ...
        if (!user) {
          user = new User({
            ...
            refreshToken: refreshToken, // Attach the refresh token
          });
          ...
          user.refreshToken = refreshToken || user.refreshToken; // Keep old refresh token if new one is missing
          await user.save();
          await cacheData.cacheUser(user);
        }
